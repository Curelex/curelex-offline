...
import { loadClinics, saveClinics } from '../utils/helpers';
...
export default function PlanSelection({ onDone }) {
  ...
  function handlePay() {
    ...
    setTimeout(() => {
      ...
      const idx = all.findIndex((c) => c.id === session.clinicId);
      if (idx !== -1) {
        const now = new Date();
        const exp = new Date(now);
        exp.setMonth(exp.getMonth() + 1);
        all[idx].plan            = selected;
        all[idx].planActivatedAt = now.toISOString().split('T')[0];
        all[idx].planExpiresAt   = exp.toISOString().split('T')[0];
        saveClinics(all);
      }
      setStep('success');
    }, 1800);
      ...
      {step === 'success' && plan && (
        <div style={{ width:'100%',maxWidth:420,textAlign:'center',animation:'fadeUp 0.45s ease' }}>
          <div style={{ background:C.white,borderRadius:24,padding:'46px 32px 36px',
            ...
            {/* ✅ KEY FIX: call onDone to go back to AdminDashboard instead of reloading */}
            <button
              onClick={onDone}
              className="pay-btn"
